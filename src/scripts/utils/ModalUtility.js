...
export class ModalUtility {
  ...
  static createAvailableGroupCard(group) {
    ...
    // Build card HTML with group info and join button
    cardDiv.innerHTML = `
  ...
  `;
    ...
    // Attach click handler for join functionality
  ...
  static async handleJoinGroup(group) {
    try {
      ...
      const memberData = {
        ...
        creator: false // User is joining, not creating
      };
      ...
      setTimeout(() => {
        ...
        modalInstance.hide();
        // Note: Page refresh should be handled by calling page, not here
      }, 1500);
  ...
  static async loadDepartments() {
    try {
      const authHeader = UserService.getAuthHeader();
      const response = await ApiService.getData('enums/', authHeader);
