...
class AwardsPage {
  ...
  buildAwardDefinitions() {
    const awardDetails = {
      ...
      3: {
        ...
        icon: 'fa-solid fa-gift text-purple',
      ...
      4: {
        ...
        icon: 'fa-solid fa-trophy text-bronze',
      ...
      5: {
        ...
        icon: 'fa-solid fa-crown text-yellow',
  ...
  async loadUserAwards() {
    try {
      ...
      // Handle both data structures like in DashboardPage
      const userId = currentUserData?.data?.id || currentUserData?.id;
      
      if (!userId) {
        console.error('No user ID found in:', currentUserData);
        return;
      }
